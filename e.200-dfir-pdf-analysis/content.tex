...
\begin{frame}[fragile]{Inspecting PDF electronic Signatures}

The tool \alert{pdfsig} can list PDF signatures and dump the embedded DER-encoded
PKCS\#7/CMS signature objects for deeper analysis.

\medskip

\begin{block}{List and dump signatures}
\begin{verbatim}
pdfsig -dump signed-by-two.pdf
\end{verbatim}
\end{block}
...
\begin{verbatim}
signed-by-two.pdf.sig0
signed-by-two.pdf.sig1
signed-by-two.pdf.sig2
\end{verbatim}


\end{frame}

\begin{frame}[fragile]{Analyze electronic signatures}
\begin{verbatim}
pdfsig  signed-by-two.pdf

Signature #2:
  - Signature Field Name: Signature1
  - Signer Certificate Common Name: REDACTED
  - Signer full Distinguished Name: CN=REDACTED,
serialNumber=REDACTED,givenName=REDACTED,SN=REDACTED,C=GR
  - Signing Time: May 22 2026 14:00:34
  - Signing Hash Algorithm: SHA-256
  - Signature Type: ETSI.CAdES.detached
  - Signed Ranges: [0 - 642079], [683041 - 691472]
...
The tool \alert{openssl} can be used to parse the signature files.

\begin{block}{Analyze dumped DER signatures with OpenSSL}
\begin{verbatim}
openssl pkcs7 -inform DER \
-in signed-by-two.pdf.sig0 \
-print -noout
\end{verbatim}
\end{block}

\begin{block}{Alternative low-level ASN.1 dump}
\begin{verbatim}
openssl asn1parse -inform DER \
    -in signed-by-two.pdf.sig0 -i
\end{verbatim}
\end{block}
...
...
   d.data: <ABSENT>
    cert:
        cert_info:
          version: 2
          serialNumber: 8646704272364946413
          signature:
            algorithm: sha256WithRSAEncryption (1.2.840.113549.1.1.11)
            parameter: NULL
          issuer: C=GR, O=BYTE Computer S.A./organizationIdentifier=VATGR-094265075, OU=BYTE-PKI, CN=BYTE KSHDE Certification Authority 001
...
\end{verbatim}
\end{block}
\end{frame}
...
Assessing which tools were used to create the signatures.

The local script with these slides called \alert{extract-tool-indication.py} can be used.

\begin{verbatim}
./extract-tool-indication.py signed-by-two.pdf

==== 2117 0 obj ====
/Filter: /FlateDecode
/SubFilter: /ETSI.CAdES.detached
/Name: /Docusign#20Signature#20Appliance
/M: (D:20260522150034+03'00')
/Prop_Build:
/App <<
/Name /Docusign#20Signature#20Appliance
