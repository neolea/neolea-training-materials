...
\section{Child Sexual Abuse Material (CSAM)}
\begin{frame}
    \frametitle{Child Sexual Abuse Material (CSAM)}
    \centerline{
        \includegraphics[scale=0.25]{images/crawled-ce.png}
    }
\end{frame}

\begin{frame}
   \frametitle{Child Sexual Abuse Material: Challenges}
   \begin{itemize}
           \item {\bf Lack of automatic exchange with law enforcement}
        \item Missing a list of keywords related to some sensitive topics such as CSAM
        \begin{itemize}
             \item Optimise the detection
             \item Could bootstrap integration of machine learning (supervised learning)
        \end{itemize}
   \end{itemize}
    ...
    \centerline{
        \begin{tabular}{l}
            $\rightarrow$ Not always optimal\\
            $\rightarrow$ Not our expertise\\
        \end{tabular}
    }
\end{frame}
